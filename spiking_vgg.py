...
class LinearSpike(torch.autograd.Function):
    ...
    def backward(ctx, grad_output):
        ...
        grad_input = grad_output.clone()
        grad = LinearSpike.gamma * F.threshold(1.0 - torch.abs(image), 0, 0)
        return grad * grad_input, None
...
class VGG_SNN_STDB(nn.Module):
    def __init__(
        ...
        self.act_func = STDB.apply if activation == "STDB" else LinearSpike.apply
        self.labels = labels
        ...
        for l, classifier in enumerate(self.classifier):
            if isinstance(classifier, nn.Linear):
                self.mem[prev + l] = torch.zeros(
                    self.batch_size, classifier.out_features
                )
            elif isinstance(classifier, nn.Dropout):
                self.mask[prev + l] = classifier(
        ...
        for t in range(self.timesteps):
            ...
            for l in range(len(self.features)):
                if isinstance(self.features[l], (nn.Conv2d)):
                    ...
                    rst = self.threshold[l] * (mem_thr > 0).float()  # reset
            ...
            if not find_max_mem:
                self.mem[prev + l + 1] += self.classifier[l + 1](out_prev)
